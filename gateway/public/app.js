...
const LOCAL_MODE = new URLSearchParams(location.search).get('local') === '1'
...
function enterPollMode() {
  if (LOCAL_MODE) {
    // Loopback mode keeps a single live WebSocket path. The embedded engine
    // and gateway have a local lifecycle, so remote polling fallback only
    // adds another timer and can hide a process restart from the native shell.
    state.streamMode = 'ws'
    setReconnect(1500)
    clearStreamRetry('mux')
    clearStreamRetry('host')
    setTimeout(() => { if (state.token) openStreams() }, 1500)
    updateConn()
    return
  }
...
function applyLocalModeUi() {
  ...
  $('settings-group-servers')?.classList.add('hidden')
  $('settings-token-row')?.classList.add('hidden')
  $('settings-bg-poll-row')?.classList.add('hidden')
  document.body.classList.add('dsh-local-mode')
}
