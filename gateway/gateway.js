...
const LOCAL_MODE = process.env.DSH_REMOTE_LOCAL === '1'
...
const ANNOUNCEMENTS_URL = LOCAL_MODE ? '' : process.env.DSH_REMOTE_ANNOUNCEMENTS_URL === undefined
...
const UPDATE_CHECK_ENABLED = !LOCAL_MODE && process.env.DSH_REMOTE_UPDATE_CHECK !== '0'
...
server.listen(PORT, HOST, () => {
  ...
  console.log('  本地:  http://127.0.0.1:' + PORT + '/?token=' + (clientToken || '未生成'))
  if (!LOCAL_MODE) for (const ip of lanAddresses()) {
    console.log('  手机(同一网络): http://' + ip + ':' + PORT + '/?token=' + (clientToken || '请在管理页创建设备密钥'))
  }
  if (!LOCAL_MODE) console.log('  管理页: http://127.0.0.1:' + PORT + '/admin')
  if (HOST === '127.0.0.1' && !LOCAL_MODE) {
